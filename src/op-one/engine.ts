...
export function createAuraCodeEngine(opts: AuraCodeEngineOptions = {}): Engine {
  ...
  return {
    async run(req: EngineRunRequest): Promise<EngineRunResult> {
      ...
      const { loadProjectContext } = await import('../agent/context.js');
      ...
      try {
        const result = await runAgentLoop({
          ...
          display: createSilentDisplay() as unknown as import('../cli/display.js').Display,
    ...
    async verify(req: EngineVerifyRequest): Promise<EngineVerifyResult> {
      const { verifyTask } = await import('../verify/index.js');
    ...
    async recordEpisode(projectRoot: string, episode: EngineEpisodeInput): Promise<void> {
      const { saveEpisode } = await import('../archimedes/episode-capture.js');
    ...
    async resolveModel(req: ResolveModelRequest): Promise<ResolvedModel> {
      ...
      const { DEFAULT_ARCHIMEDES_CONFIG } = await import('../archimedes/types.js');
      ...
      const { assessCompetence } = await import('../archimedes/competence.js');
      const { loadEpisodes } = await import('../archimedes/episode-capture.js');
      try {
