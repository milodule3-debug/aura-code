...
> Status: MVP design, implemented under `src/op-one/`.
...
One seam, declared in `src/op-one/engine.ts`:
...
Everything in `src/op-one/` depends on this interface, not on `aura-code` internals.
...
Legal transitions (enforced in code, `src/op-one/verification.ts`):
...
stop condition (duplicating aura-code), the second is premature. The `Engine` seam is
the extraction plan: once the interfaces in §14 are exported from a published
`aura-code` package, `src/op-one/` moves out unchanged and depends on that package.
Package/application id is already `aura-op-one`.
